...
export const MainDashboardSummary = ({ 
  onNavigate, 
  onOpenModalUser, 
  onOpenModalStaff, 
  onOpenModalEvent 
  ...
  const isCoach = currentUser?.rol === 'coach';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-900 to-amber-950/40 border border-slate-800 p-5 sm:p-6 rounded-3xl shadow-2xl relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="relative z-10">
          ...
          <p className="text-xs text-slate-400">
            Bienvenido, <strong className="text-white">{currentUser?.nombre} {currentUser?.apellido}</strong> ({currentUser?.rol?.toUpperCase()})
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 relative z-10 w-full sm:w-auto">
          {onOpenModalUser && (
            <button
              ...
              className="bg-amber-500 hover:bg-amber-600 text-slate-950 font-black px-4 py-2.5 rounded-2xl text-xs flex items-center justify-center gap-1.5 shadow-lg shadow-amber-500/20 transition-all hover:scale-105 cursor-pointer flex-1 sm:flex-initial"
            >
          ...
          <button
            ...
            className="bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold px-4 py-2.5 rounded-2xl text-xs flex items-center justify-center gap-1.5 border border-slate-700 transition-all hover:scale-105 cursor-pointer flex-1 sm:flex-initial"
          >
            <Share2 className="w-4 h-4 text-emerald-400" /> {copiedLink ? '¡Link Copiado!' : 'Copiar Link Registros'}
          </button>
          ...
          {isCoach && onOpenModalEvent && (
            <button
              onClick={onOpenModalEvent}
              className="bg-red-500 hover:bg-red-600 text-white font-extrabold px-4 py-2.5 rounded-2xl text-xs flex items-center justify-center gap-1.5 shadow-lg shadow-red-500/20 transition-all hover:scale-105 cursor-pointer flex-1 sm:flex-initial"
            >
              + Partido / Convocatoria
            </button>
          )}
        </div>
      </div>

      {/* Grid de Tarjetas Interactivas de Resumen (Role-based) */}
      <div className={`grid grid-cols-1 ${isStaffAdmin ? 'sm:grid-cols-2 lg:grid-cols-4' : 'sm:grid-cols-2 lg:grid-cols-3'} gap-5`}>
